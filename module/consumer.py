    ...
    def start_consuming(self):
        ...
        while 'NextShardIterator' in record_response:
            ...
            if record_response['Records']:
                ...
                data = data.join(data['text'].apply(lambda t: self.comprehend_tweet(t)).apply(pd.Series)).rename(columns=dict(zip(range(5), ['mixed', 'negative', 'neutral', 'positive', 'label'])))
                print(data)
...
if __name__ == "__main__":
    ...
    files = config.read(['config/secret.ini', 'config/app.yaml'])
    print(files)
